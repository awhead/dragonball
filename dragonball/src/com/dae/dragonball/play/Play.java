...
public class Play {
    ...
    public static void inputAction(PlayerVO player) {
        ...
        if (player.isPlayerBot() == false) {
            ...
            try {
                player.setAction(Integer.parseInt(temp));
                ;
            } catch (Exception e) {
    ...
    public static void printMessage(PlayerVO player) {
        ...
        if (player.isHpStatus() == true) {
            System.out.println("Player [" + player.getPlayerName() + "\t]\t//체력 감소/////////");
        }
        if (player.getHp() == 0) {
            System.out.println("Player [" + player.getPlayerName() + "\t]\t/사망////////////");
        }
        if (player.isWin()) {
            System.out.println("Player [" + player.getPlayerName() + "\t]\t++승리++++++++++++++");
        }
